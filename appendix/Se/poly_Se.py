...
pse4_recipe = PolyhedraRecipe(central_atoms='P', vertex_atoms='Se', method='nearest neighbours',
                            n_neighbours=4)
...
f, ax = plt.subplots(n, 1, sharey=True, figsize=(15,n), gridspec_kw={'hspace': 0.1})
...
ax[3].set_xlabel('Time / fs')
f.text(0.09, 0.5, 'Closest Orientation Number', va='center', rotation='vertical')
for i in range(3):
    ax[i].set_xticks([])
...
with open('orientation_1.dat', 'w+') as f:
    f.write('β-PS4 Orientation Changes' + ' ' + str(type_1_rotation_count) + '\n'
            + 'β-PS4 Tetrahedra' + ' ' + str(type_1_poly) + '\n'
            + 'Mean β-PS4 Rotation Frequency' + ' ' + str(type_1_rotation_count/type_1_poly) + ' '
            + 'fs\N{SUPERSCRIPT MINUS}\N{SUPERSCRIPT ONE}')
